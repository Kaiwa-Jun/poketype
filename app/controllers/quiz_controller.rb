...
def new
  ...
  # real_pokemons の選択条件を調整（fake_type_idも考慮）
  real_pokemons = Pokemon.where('(type_id = ? OR secondary_type_id = ?)', @type.id, @type.id)
                          .where('fake_type_id IS NULL OR fake_type_id != ?', @type.id)
                          .order('RAND()').limit(6).to_a

  # fake_pokemons の選択に fake_type_id を考慮
  fake_pokemons = Pokemon.where(fake_type_id: @type.id, is_fake: true).order('RAND()').limit(3).to_a

  if fake_pokemons.count < 3
    extra_real_pokemons = Pokemon.where(type_id: @type.id)
                                 .where('is_fake = ? OR is_fake IS NULL', false)
                                 .where.not(id: real_pokemons.map(&:id))
                                 .order('RAND()').limit(3 - fake_pokemons.count).to_a
